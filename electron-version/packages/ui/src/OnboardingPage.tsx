...
// The official one-line installer is a Unix shell script, so it cannot run on
// Windows. There we point users at a manual `cargo install` instead and hide
// the in-app "Install" button (the GUI cannot run a shell installer on Windows).
const INSTALL_COMMAND = IS_WINDOWS
  ? "cargo install --git https://github.com/xai-org/grok-build"
    ...
    <div className="flex h-full w-full items-center justify-center bg-background p-8">
      <div className="flex w-full max-w-md flex-col items-center text-center">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          {stage === "missing-cli"
            ? <>{IS_WINDOWS
              ? (english ? `${__APP_NAME__} needs the Grok Build CLI. On Windows there is no one-click installer — run the command below in a terminal (Rust required), make sure grok is on your PATH, then press “Check again”.` : `${__APP_NAME__} 需要 Grok Build CLI。Windows 没有一键安装器——请在终端运行下方命令（需先装 Rust），确保 grok 在 PATH 中，然后点“重新检测”。`)
        ...
        <div className="mt-6 w-full rounded-xl border border-border bg-card p-4 text-left shadow-sm">
          {stage === "missing-cli" ? <>
            ...
            {IS_WINDOWS ? (
              <p className="mt-3 rounded-lg border border-border bg-secondary/50 px-3 py-2.5 text-left text-[11px] leading-relaxed text-muted-foreground">
                {english
                  ? "Run the command above in PowerShell or CMD (install Rust first if needed). After it finishes, confirm grok is on your PATH, then press “Check again and continue”."
                  : "请在 PowerShell 或 CMD 中运行上方命令（如未装 Rust 需先安装）。完成后确认 grok 在 PATH 中，再点“重新检测并继续”。"}
              </p>
            ) : (
            ...
            {showApiKey && <form className="mt-2" onSubmit={(event) => { event.preventDefault(); void run("api-key", async () => { await onSaveApiKey(apiKey); setApiKey(""); }); }}>
              ...
              <button type="submit" disabled={!!working || !apiKey.trim()} className="mt-2 flex w-full items-center justify-center gap-2 rounded-md bg-secondary px-3 py-2 text-[12px] font-medium text-foreground disabled:opacity-60"><ShieldCheck size={13} /> {working === "api-key" ? (english ? "Saving securely and verifying…" : "正在安全保存并验证…") : (english ? "Save securely and connect" : "安全保存并连接")}</button>
            </form>}
        ...
        <p className="mt-4 text-[11px] leading-relaxed text-muted-foreground/80">
          {IS_WINDOWS
            ? (english ? <>Make sure <span className="font-mono">grok</span> is on your PATH; API keys are stored in the Windows credential store.</> : <>请确保 <span className="font-mono">grok</span> 在 PATH 中；API Key 存于 Windows 凭据管理器。</>)
