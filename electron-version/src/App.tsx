...
export default function App() {
  ...
  // macOS hiddenInset title bar floats the traffic lights over the webview,
  // so left-anchored layouts need top padding to clear them. Centered layouts
  // (boot / error / language picker / onboarding) leave the top-left empty,
  // so the lights never overlap content there and need no padding.
  const insetPad = (window as any).electron?.platform === "darwin"
    ? { paddingTop: 38 }
    : undefined;
  ...
  if (screen === "settings") {
    return <div className="h-full w-full" style={insetPad}><SettingsPage /></div>;
  }

  return (
    <div className="flex h-full w-full overflow-hidden bg-background text-foreground" style={insetPad}>
      <Sidebar />
